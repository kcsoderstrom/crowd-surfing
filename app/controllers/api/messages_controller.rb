    ...
    def index
      if current_user
        ...
        @received_messages = Message.find_by_sql(<<-SQL
          SELECT received_messages.*, profiles.primary_photo_id AS photo_id, profiles.name AS sender_name, profiles.id AS sender_id, profiles.location AS location
          FROM (
        ...
        @sent_messages = Message.find_by_sql(<<-SQL
          SELECT sent_messages.*, profiles.primary_photo_id AS photo_id, profiles.name AS receiver_name, profiles.id AS receiver_id, profiles.location AS location
          FROM (
            SELECT * FROM messages
